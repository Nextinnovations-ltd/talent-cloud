...
import exploreArrow from "@/assets/Group 2292.svg";
...
/* useEffect(() => {
    ...
    const sketch = (p) => {
      p.setup = () => {
        ...
        canvasDom.style.boxShadow = '0px 1px 3px rgba(166, 175, 195, 0.40)';
        canvasDom.style.borderRadius = '8px';
        canvasDom.style.backgroundColor = '#ffffff';
      ...
      const initPhysics = () => {
        ...
        // Static walls around the canvas edges to contain words
        ...
        const mouse = Matter.Mouse.create(canvasDom);
        mouseConstraint = Matter.MouseConstraint.create(engine, {
          mouse,
          constraint: {
            stiffness: 0.2,
            render: { visible: false },
            // Make sure the dragged body cannot be pulled too far away
        ...
        // Release dragged body if mouse leaves the canvas (drop it)
        ...
        // Also release dragged body on mouse up anywhere
      ...
      const createWords = () => {
        ...
        words = wordsToDisplay.map(w =>
          new Word(p.random(paddingX, width - paddingX), p.random(paddingY, height / 2), w)
        );
      };
      ...
      class Word {
        ...
        show() {
          ...
          // Keep bodies inside canvas with bounce
          if (x < margin) {
            Matter.Body.setVelocity(this.body, { x: Math.abs(vx), y: vy });
            Matter.Body.setPosition(this.body, { x: margin, y });
          } else if (x > width - margin) {
            Matter.Body.setVelocity(this.body, { x: -Math.abs(vx), y: vy });
            Matter.Body.setPosition(this.body, { x: width - margin, y });
          }
          if (y < margin) {
            Matter.Body.setVelocity(this.body, { x: vx, y: Math.abs(vy) });
            Matter.Body.setPosition(this.body, { x, y: margin });
          } else if (y > height - margin) {
            // If dragged below canvas bottom, bounce up but also add downward force to simulate drop
            Matter.Body.setVelocity(this.body, { x: vx, y: -Math.abs(vy) * 0.5 });
            Matter.Body.setPosition(this.body, { x, y: height - margin });
          }
      ...
      p.draw = () => {
        ...
        // If mouse drag point is beyond canvas, release dragged body
        if (mouseConstraint.body) {
  ...
  useEffect(() => {
    ...
    const sketch = (p) => {
      ...
      const initPhysics = () => {
        ...
        // Thicker walls extending slightly beyond canvas to fully contain bodies
        const wallThickness = 50;
        const wallOptions = {
          isStatic: true,
          restitution: 0.9, // bouncier
          friction: 0.7,    // enough friction to reduce sliding jitter
          frictionStatic: 0.5,
        };

        const bottomWall = Matter.Bodies.rectangle(width / 2, height + wallThickness / 2, width + wallThickness * 2, wallThickness, wallOptions);
        const topWall = Matter.Bodies.rectangle(width / 2, -wallThickness / 2, width + wallThickness * 2, wallThickness, wallOptions);
        const leftWall = Matter.Bodies.rectangle(-wallThickness / 2, height / 2, wallThickness, height + wallThickness * 2, wallOptions);
        const rightWall = Matter.Bodies.rectangle(width + wallThickness / 2, height / 2, wallThickness, height + wallThickness * 2, wallOptions);

        Matter.World.add(engine.world, [bottomWall, topWall, leftWall, rightWall]);
        ...
        mouseConstraint = Matter.MouseConstraint.create(engine, {
          mouse,
          constraint: {
            stiffness: 0.15,    // softer spring for less snapping force
            damping: 0.1,       // reduce oscillations during drag
            render: { visible: false },
            angularStiffness: 0.05,
            length: 20,         // allow slight drag "distance" for smoothness
          }
        });

        Matter.World.add(engine.world, mouseConstraint);

        // Release dragged body if mouse leaves canvas - with debounce
        let releaseTimeout;
        canvasDom.addEventListener("mouseleave", () => {
          ...
          }, 100); // small delay to avoid abrupt release
        });
      ...
      class Word {
        constructor(x, y, txt) {
          ...
          this.body = Matter.Bodies.rectangle(x, y, w, h, {
            friction: 0.6,
            restitution: 0.9,  // high bounce for lively movement
            density: 0.01,
            frictionAir: 0.04,
            chamfer: { radius: 6 }
          });
      ...
      p.draw = () => {
        ...
        // Release drag if mouse moves too far outside canvas (preventing strong pull forces)
        if (mouseConstraint.body) {
          const mousePos = mouseConstraint.mouse.position;
          const margin = 100;
          if (
            mousePos.x < -margin || mousePos.x > width + margin ||
            mousePos.y < -margin || mousePos.y > height + margin
          ) {
            mouseConstraint.body = null;
          }
        }

        // Clamp max velocity to reduce jitter when bouncing
        const maxVel = 15;
        words.forEach(w => {
          ...
          if (
            ...
            Matter.Body.setVelocity(w.body, {
              x: Math.sign(v.x) * Math.min(Math.abs(v.x), maxVel),
              y: Math.sign(v.y) * Math.min(Math.abs(v.y), maxVel),
            });
          }
        });

        words.forEach(w => w.show());
      };
    };
  ...
  <div className="h-[600px] relative bg-[linear-gradient(to_bottom,_#75d1ffe3_90%,_#F7F7F7_100%)]">
    ...
    <div className="">
        ...
        <div className="pt-[66px] relative z-10">
          <h1 className="text-black text-[64px] font-bold leading-[87px]">
          ...
          <div className="flex justify-center mt-[80px]">
            <Button className="bg-[#0389FF] hover:opacity-75 rounded-[26px] p-[10px] w-[205px] h-[58px] text-[20px] font-bold text-white flex items-center gap-[10px]">
              Explore Jobs
