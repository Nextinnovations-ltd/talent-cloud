...
const ImagePicker = ({ setPreview, preview, form, setIsOpen, type = "circle", imageUploadType = "profile"}: {
  ...
  return (
    <>
      <div className="flex gap-[30px] items-center">
        <div
          {...getRootProps()}
          className={`cursor-pointer bg-slate-200/50 flex items-center justify-center  transition-all duration-300 w-[110px] h-[110px] ${type === "circle" ? "rounded-full" : "rounded-[16px]"
            }`}
        >
          {preview ? (
            <img
              ...
              width={110}
              height={110}
              className={`object-cover w-full h-full ${type === "circle" ? "rounded-full" : "rounded-[16px]"
                }`}
            />
          ) : (
