...
export const SkillsSection: React.FC<SkillsSectionProps> = ({ skills }) => {
  ...
  return (
    <div className="flex items-start mb-[50px] gap-2">
      <img width={24} height={24} src={SKILLS} alt="Skills" className='mt-1'/>
      <div className="text-[#7e6868] w-full flex flex-wrap gap-2 items-center">
        {displayedSkills.map((item, index) => (
          <Badge
            key={index}
            className="border inline-block capitalize  px-[10px] bg-[#F2F2F2] rounded-[8px] py-[4px] text-[14px] font-normal text-black"
          >
            {item}
          </Badge>
        ))}
