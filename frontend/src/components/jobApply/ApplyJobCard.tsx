...
const ApplyJobCard: React.FC<ApplyJobCardProps> = ({ job, onClick, isSelected = false }) => {
  ...
  return (
    <div className="relative w-[400px] mx-auto">
      <div
        ref={cardRef}
        className={`p-[30px] border-[#CBD5E1B2] duration-700 relative border-[2px] cursor-pointer min-h-[429px] w-[400px] rounded-[17px] transition-colors flex flex-col z-0 ${
          isSelected ? " border-blue-500 border-[3px] " : "hover:border-blue-500"
        } ${job?.is_expired && 'opacity-60'}`}
        onClick={() => onClick(job)}
      >
      ...
      {/* Absolutely positioned Expired text outside faded card */}
      {job?.is_expired && (
        <h3 className="text-red-500 z-50 absolute bottom-6 right-6 bg-white px-2 py-1   opacity-100" style={{ opacity: 1 }}>
          Expired
        </h3>
      )}
    </div>
