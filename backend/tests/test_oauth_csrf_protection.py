...
class OAuthValidatorCSRFTestCase(TestCase):
    ...
    def test_validate_state_parameter_invalid_format(self):
        """Test state parameter validation rejects invalid formats"""
        invalid_states = [
            None,  # None
            "",  # Empty
            "no_dots_here",  # No dots
            "one.dot",  # Only one dot
            "too.many.dots.here.invalid",  # Too many dots
            "valid.format.but\x00null",  # Contains null byte
            "valid.format.but\nnewline",  # Contains newline
            "special!chars.not@allowed.1234567890",  # Invalid characters
            "a" * 250,  # Too long
        ]
        
        for invalid_state in invalid_states:
            with self.assertRaises(ValidationError):
                OAuthValidator.validate_state_parameter(invalid_state)
