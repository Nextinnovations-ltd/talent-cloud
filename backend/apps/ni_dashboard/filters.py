...
class FavouriteJobSeekerOrderingFilter(OrderingFilter):
     """
     Custom ordering filter that handles experience_years and open_to_work sorting
     """
     ...
     def _add_experience_annotation(self, queryset):
          ...
          try:
               return queryset.annotate(
                    experience_years=Case(
                         When(
                         user__occupation__experience_years__isnull=False,
                         then=F('user__occupation__experience_years')
                         ),
                         default=0,
                         output_field=FloatField()
                    )
     ...
     def _add_open_to_work_annotation(self, queryset):
          ...
          try:
               return queryset.annotate(
                    open_to_work_sort=Case(
                         When(user__is_open_to_work=True, then=1),
                         When(user__is_open_to_work=False, then=0),
                         default=0,
                         output_field=IntegerField()
                    )
               )
